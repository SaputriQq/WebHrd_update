...
<%
   ...
   nip=request.QueryString("nipatasan")
    'response.write cabang &"<br>"
    ...
    pkaryawan.commandText = "SELECT Kry_Nip, Kry_Nama FROM HRD_M_Karyawan WHERE Kry_AgenID = '"& cabang &"'ORDER BY Kry_Nama ASC"
     'response.write pkaryawan.commandText &"<br>"
    ...
    response.write pdivisi.commandText &"<br>"
    set pdivisi = pdivisi.execute

    ' set shiftupdate=server.createobject("ADODB.Command")
    ' shiftupdate.activeConnection = MM_Cargo_string

    ' shiftupdate.commandText ="SELECT * FROM HRD_M_Karyawan WHERE Shf_NIP ='"&nip"'"
...
<html lang="en">
  ...
  <body>
   <div class="container ">
        <div class="row mt-4">
            <div class="col-md">
                <h1 class="text-center">FORM SETTING SHIFT</h1>
            </div>
        </div>
        <%
            no=0
            ...
            no=no+1
        %>
        <input id="nipke(<%=no%>)" value="<%=ptanggal("Shf_NIP")%>">
        <%ptanggal.movenext
        loop
        nomor = no%>
        <input id="nomor" value="<%=nomor%>">
                    ...
                    <div class="col-auto">
                        <div class="col-12">
                            <div class="col-auto">
                                ...
                                <% do while not pagen.eof %>
                                <input type="hidden" id="agenid" class="form-control" value="<%=pagen("Agen_ID")%>" name ="agenid">
                                <fieldset disabled>
                                    <input type="t
                                    ext" id="namaagen" class="form-control" value="<%=pagen("Agen_Nama")%>" name="namaagen">
